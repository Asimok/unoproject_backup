...
const long temperature_interval = 120000;       //温度传感器采集时间间隔
const long gps_interval = 30000;       //GPS采集时间间隔
const long light_interval = 60000;    //光线传感器采集时间间隔
const long mpu_interval = 2000;    //mpu传感器采集时间间隔

//const long temperature_interval = 20000;       //温度传感器采集时间间隔
...
void fun_temperature_sensor() {
  ...
  if (currentMillis - previousMillis_temperature >= temperature_interval) //如果和前次时间大于等于时间间隔
  {
    ...
    if (currentMillis % gps_interval == 0 || currentMillis % light_interval == 0 || currentMillis % mpu_interval == 0)
      delay(1000);
...
void fun_light_sensor() {
  ...
  if (currentMillis - previousMillis_light >= light_interval) //如果和前次时间大于等于时间间隔
  {
    ...
    if (currentMillis % gps_interval == 0 || currentMillis % temperature_interval == 0 || currentMillis % mpu_interval == 0)
      delay(1000);
...
void  fun_gps_sensor()
{
  ...
  if (currentMillis - previousMillis_gps >= gps_interval) //如果和前次时间大于等于时间间隔
  {
    ...
    if (currentMillis % light_interval == 0 || currentMillis % temperature_interval == 0 || currentMillis % mpu_interval == 0)
      delay(1000);
...
void fun_mpu6050_sensor() {
  ...
  if (currentMillis - previousMillis_mpu >= mpu_interval) //如果和前次时间大于等于时间间隔
  {
    ...
    if (currentMillis % gps_interval == 0 || currentMillis % temperature_interval == 0 || currentMillis % light_interval == 0)
      delay(1000);
